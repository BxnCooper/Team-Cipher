...
def get_profile(user_id):
    """VULNERABILITY: IDOR - no auth check, anyone can view any profile including sensitive data"""
    try:
        user = get_user_by_id(user_id)
        if not user:
        ...
        # VULNERABILITY: Information Disclosure - returns password, role, email
        return jsonify({
            'user': user,
            'listings': listings
        })
...
def create_listing():
    """VULNERABILITY: File Upload - no file type validation, accepts anything"""
    try:
...
def send_message():
    """VULNERABILITY: No CSRF, no auth check"""
    try:
        data = request.get_json()
